ase <-
function(Y1, Y2, Z, output.tag, p.cut, min.nTotal=10, min.N=10, 
  min.Nhet=10, cis.only=TRUE,  cis.distance=2e+05, 
  eChr=NULL, ePos=NULL, mChr=NULL, mPos=NULL, trace=0)
{
  ...
  if(min.N < min.Nhet){
    stop("min.N should not be smaller than min.Nhet\n")
  }
  ...
  if(cis.only){
    ...
    warning("ASE mapping requires haplotyp information, which may be inaccruate for non-cis eQTL\n")
  }
