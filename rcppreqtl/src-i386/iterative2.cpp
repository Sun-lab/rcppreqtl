...
double negLogH0 (int n, double* para, void* ex, SEXP x1){
  ...
  cOptStruct OS = (cOptStruct) ex;
  exPara = (double *)(OS->extra);
//  exPara = (double *) ex;
  N      = ceil(exPara[0]-0.5);
  h      = ceil(exPara[1]-0.5);
  pi0    = exPara[6];
 ...
 *
  ...
  T       Expression of total read counts
             ...
             int* trace, int* succeed)
{
  ...
  double *exPara, *nA, *nTotal, *zeta;
  double ndeps[2] = {1e-3,1e-3};
  cOptStruct OS;
  printf("flag2\n");
//  OS = (pOptStruct) calloc(1,sizeof(opt_struct1));//R_alloc?
  OS = (cOptStruct) R_alloc(1,sizeof(opt_struct2));//R_alloc?
  printf("flag3\n");
  int nY = dims[0];
  int nZ = dims[1];
  int N  = dims[2];
...
//  OS->R_env = rho;
  ...
  npara   = 2;  //number of parameters to be optimized
  lmm     = 5;  //is an integer giving the number of BFGS updates retained in the "L-BFGS-B" method, It defaults to
  fail    = 0;
  failA   = 0;
  failB   = 0;
  fncount = 0;
  ...
  for(i=0; i<nY; i++,pY1+=N,pY2+=N){
    ...
    lbfgsb1(npara, lmm, initPara, lower, upper, nbd, &Fmin, 
           negLogH0, negGradLogH0, &failA, (void*)OS, factr, pgtol,  
           &fncount, &grcount, maxit, msg, 0, nREPORT, wa, iwa, g1,x1);
    Rprintf("\n  Obtained MLE for H0: twologlik0=%.4e", -Fmin);
    Rprintf(" theta0A=%.4e", initPara[0]);
    initPara[0] = 0.1; 
    lbfgsb1(npara, lmm, initPara, lower, upper, nbd, &Fmin, 
           negLogH0, negGradLogH0, &failA, (void*)OS, factr, pgtol,  
           &fncount, &grcount, maxit, msg, 0, nREPORT, wa, iwa, g1,x1);
    Rprintf("\n  using numeric grad Obtained MLE for H0: twologlik0=%.4e", -Fmin);
    Rprintf(" theta0A=%.4e", initPara[0]);
    ...
    for(j=0; j<nZ; j++,pZ+=N){
      ...
      //       &fncount, &grcount, maxit, msg, 0, nREPORT);
      lbfgsb1(npara, lmm, initPara, lower, upper, nbd, &Fmin, 
             negLogH1, negGradLogH1, &fail, (void*)OS, factr, pgtol,  
             &fncount, &grcount, maxit, msg, 0, nREPORT, wa, iwa, g1,x1);
      ...
      if(*trace > 1){
        ...
        negGradLogH1(2, initPara, gr, (void*)OS,x1);
