...
//    for (size_t i = 0; i != idx.size(); ++i){
    for (size_t i = 0; i < nind; i++){
      idx[i] = v[i].first;
//      std::cout << i << " " << v[i].first << " " << idx[i] << std::endl;
    }    
...
//    std::cout << "GGCP ";    
  ...
  Vector<Dtype> &SubspaceMin) {
    ...
    // STEP 2: "v = t(W)*Z*rc" and STEP 3: "v = M*v"
    Vector<Dtype> v = M * (WZ * r);
    // STEP 4: N = 1/theta*t(W)*Z*(t(W)*Z)^T
...
//        std::cout << "ncol= "<< yHistory.cols() << " " << sHistory.cols() << "     ";
...
//        std::cout << "ncol(A) = " << A.cols() << "     " << "ncol(D) = " << D.cols() << "     ";
//        std::cout << "ncol(L) = " << L.cols() << "     " << "nrow(L) = " << L.rows() << "     ";
//        std::cout << "ncol(sH) = " << (sHistory.transpose() * sHistory).cols() << "     ";
        MM << D, L.transpose(), L, ((sHistory.transpose() * sHistory) * theta);
//        MM << D, L.transpose(), L, (C * theta);
//        std::cout << "ncol(MN) = " << MM.cols() << "     ";
//       std::cout << std::setprecision(5) << "A = " << std::endl << A <<std::endl;
//       std::cout << std::setprecision(5) << "L = " << std::endl << L <<std::endl;       
//       std::cout << std::setprecision(5) << "D = " << std::endl << D <<std::endl;       
//       std::cout << std::setprecision(5) << "MM = " << std::endl << MM <<std::endl;       
        M = MM.inverse();
//       std::cout << std::setprecision(5) << "M = " << std::endl << M <<std::endl;       
